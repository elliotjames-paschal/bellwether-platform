#!/usr/bin/env python3
"""
Reconstruct tickers for all party winner markets using API ticker probing
Extends House/Senate to include Attorney General, Secretary of State, Lt. Governor
"""
...
INPUT_FILE = f"{DATA_DIR}/kalshi_official_with_corrected_house_senate.csv"
OUTPUT_FILE = f"{DATA_DIR}/kalshi_official_with_party_winner_tickers.csv"
CHECKPOINT_FILE = f"{DATA_DIR}/party_winner_ticker_checkpoint.json"
...
print("=" * 80)
print("RECONSTRUCTING PARTY WINNER TICKERS")
print("=" * 80)
...
# Filter to US party winner offices
us_elections = df[df['country'] == 'United States'].copy()
party_winner_offices = ['House', 'Senate', 'Attorney General', 'Secretary of State', 'Lt. Governor']
party_markets = us_elections[us_elections['office'].isin(party_winner_offices)].copy()

print(f"✓ US elections: {len(us_elections):,}")
print(f"✓ Party winner offices total: {len(party_markets):,}")

# Filter to markets needing reconstruction (no ticker or KX-prefix)
needs_reconstruction = party_markets[
    party_markets['ticker'].isna() |
    party_markets['ticker'].str.startswith('KX', na=False)
].copy()

print(f"✓ Markets needing reconstruction: {len(needs_reconstruction):,}")
...
remaining = needs_reconstruction[~needs_reconstruction.index.isin(processed_indices)].copy()

print(f"\nTotal needing reconstruction: {len(needs_reconstruction):,}")
print(f"Already processed: {len(processed_indices):,}")
print(f"Remaining: {len(remaining):,}")

# Show breakdown by office
print(f"\nRemaining by office:")
for office in party_winner_offices:
    count = len(remaining[remaining['office'] == office])
    if count > 0:
...
def probe_ticker(office, location, current_year):
    ...
    if office == 'House':
        ...
        ticker_base = f"KXATTYGEN{state_code}"

    elif office == 'Secretary of State':
        # Get state code from location
        if pd.isna(location):
            return None, None, "no_location"
        ...
        ticker_base = f"KXSECSTATE{state_code}"

    elif office == 'Lt. Governor':
        # Get state code from location
        if pd.isna(location):
            return None, None, "no_location"
        ...
        ticker_base = f"KXLTGOV{state_code}"
...
# Process markets
if len(remaining) == 0:
    print("\n✓ All party winner markets already processed!")
else:
    print(f"\n{'=' * 80}")
    ...
    # Track by office
    office_stats = {office: {'success': 0, 'failed': 0} for office in party_winner_offices}
    ...
    for idx, row in remaining.iterrows():
        ...
        if status == "success":
            ...
            success_count += 1
            office_stats[office]['success'] += 1
        else:
            print(f"  ⚠ {status}")
            failed_count += 1
            office_stats[office]['failed'] += 1
...
print(f"✓ Saved {len(df):,} markets with reconstructed party winner tickers")
...
party_markets_updated = us_elections_updated[us_elections_updated['office'].isin(party_winner_offices)].copy()

print(f"\nParty winner markets: {len(party_markets_updated):,}")
print(f"Successfully probed: {success_count:,}")
print(f"Failed to probe: {failed_count:,}")

if len(party_markets_updated) > 0:
    with_tickers = party_markets_updated['ticker'].notna().sum()
    with_correct_tickers = party_markets_updated[
        ~party_markets_updated['ticker'].str.startswith('KX', na=False) |
        party_markets_updated['ticker'].str.startswith('KXATTYGEN', na=False) |
        party_markets_updated['ticker'].str.startswith('KXSECSTATE', na=False) |
        party_markets_updated['ticker'].str.startswith('KXLTGOV', na=False)
    ].shape[0]

    print(f"\nMarkets with tickers: {with_tickers:,} ({with_tickers/len(party_markets_updated)*100:.1f}%)")
    print(f"Markets with correct tickers: {with_correct_tickers:,} ({with_correct_tickers/len(party_markets_updated)*100:.1f}%)")

    print(f"\nBreakdown by office:")
    for office in party_winner_offices:
        office_markets = party_markets_updated[party_markets_updated['office'] == office]
        if len(office_markets) > 0:
            correct = office_markets[
                (office_markets['ticker'].str.startswith('HOUSE', na=False)) |
                (office_markets['ticker'].str.startswith('SENATE', na=False)) |
                (office_markets['ticker'].str.startswith('KXATTYGEN', na=False)) |
                (office_markets['ticker'].str.startswith('KXSECSTATE', na=False)) |
                (office_markets['ticker'].str.startswith('KXLTGOV', na=False))
            ].shape[0]
            print(f"  {office}: {correct}/{len(office_markets)} ({correct/len(office_markets)*100:.1f}%)")
