#!/usr/bin/env python3
"""
Calibration Density Plot Visualizations
Creates two calibration analysis plots:
...
print("="*80)
print("CALIBRATION DENSITY PLOT ANALYSIS (POLYMARKET + KALSHI)")
print("="*80)
...
print(f"✓ Filtered to {len(df):,} valid predictions")
...
print(f"\n{'='*80}")
print("GRAPH 1: 1-Day Before Calibration (Quantile Bins)")
print(f"{'='*80}")
...
ax.set_title('Calibration Analysis: 1 Day Before Resolution\n(Quantile Binning - Equal Sample Sizes)',
...
ax.text(0.98, 0.02, f'Total Predictions: {len(df_1day):,}',
...
plt.tight_layout()
output_file_1 = f"{GRAPH_DIR}/calibration_1day_quantile_bins.png"
...
# ============================================================================
# GRAPH 2: Calibration Distribution (1 Day Before Resolution)
# ============================================================================

print(f"\n{'='*80}")
print("GRAPH 2: Calibration Distribution (1 Day Before)")
print(f"{'='*80}")

# Filter to 1 day before
df_1day_dist = df[df['days_before_event'] == 1].copy()
print(f"Total records at 1 day before: {len(df_1day_dist):,}")
...
ax.set_title('Calibration Distribution 1 Day Before Resolution\n(Showing Bimodality Pattern)',
             fontsize=16, fontweight='bold', pad=20)
ax.set_xlim(-0.05, 1.05)
...
plt.tight_layout()
output_file_2 = f"{GRAPH_DIR}/calibration_timeseries_density.png"
...
print("SUMMARY")
print(f"{'='*80}")
print(f"Total unique markets: {df['market_id'].nunique():,}")
print(f"Total valid predictions: {len(df):,}")
print(f"Predictions at 1 day before: {len(df_1day):,}")
print(f"\nGraphs saved to: {GRAPH_DIR}")
print(f"  1. calibration_1day_quantile_bins.png")
print(f"  2. calibration_timeseries_density.png")
print(f"{'='*80}")
